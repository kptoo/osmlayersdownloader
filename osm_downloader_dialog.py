...
from qgis.PyQt.QtWidgets import (QDockWidget, QWidget, QVBoxLayout, QHBoxLayout,
                                 ...
                                 QDialog, QComboBox, QDialogButtonBox, QScrollArea, QRadioButton)
...
class OSMDownloaderDialog(QDockWidget):
    ...
    def __init__(self, iface):
        ...
        self.frame_layer = None  # For visualization on map
    ...
    def reset_plugin_state(self):
        ...
        self.frame_geometry = None
        self.frame_orientation = "portrait"
        self.remove_frame_from_map()
        if hasattr(self, 'use_frame'):
            self.use_frame.setChecked(False)
            self.draw_frame_btn.setEnabled(False)
        if hasattr(self, 'frame_portrait_radio'):
            self.frame_portrait_radio.setChecked(True)
    ...
    def setup_ui(self):
        ...
        # Frame option group
        frame_group = QGroupBox("📐 Frame Options (11 × 14 inches)")
        ...
        self.use_frame = QCheckBox("🖼️ Restrict features to frame")
        self.use_frame.setToolTip("Download only features within a 11×14 inch frame centered on searched location")
        self.use_frame.stateChanged.connect(self.on_frame_enabled_changed)
        ...
        """)
        frame_layout.addWidget(self.use_frame)
        
        # Orientation selection
        orientation_layout = QHBoxLayout()
        orientation_layout.addWidget(QLabel("Orientation:"))
        
        self.frame_portrait_radio = QRadioButton("📏 Portrait (11w × 14h)")
        self.frame_portrait_radio.setChecked(True)
        self.frame_portrait_radio.toggled.connect(self.on_frame_orientation_changed)
        self.frame_portrait_radio.setEnabled(False)
        orientation_layout.addWidget(self.frame_portrait_radio)
        
        self.frame_landscape_radio = QRadioButton("📐 Landscape (14w × 11h)")
        self.frame_landscape_radio.toggled.connect(self.on_frame_orientation_changed)
        self.frame_landscape_radio.setEnabled(False)
        orientation_layout.addWidget(self.frame_landscape_radio)
        
        orientation_layout.addStretch()
        frame_layout.addLayout(orientation_layout)
        
        # Draw frame button
        self.draw_frame_btn = QPushButton("✏️ Draw Frame on Map")
        self.draw_frame_btn.clicked.connect(self.draw_frame_on_map)
        self.draw_frame_btn.setEnabled(False)
        self.draw_frame_btn.setStyleSheet("""
            QPushButton {
        ...
        """)
        frame_layout.addWidget(self.draw_frame_btn)
        
        place_layout.addWidget(frame_group)
    ...
    def on_frame_enabled_changed(self, state):
        """Handle frame checkbox state change"""
        self.frame_enabled = state == Qt.Checked
        self.frame_portrait_radio.setEnabled(self.frame_enabled)
        self.frame_landscape_radio.setEnabled(self.frame_enabled)
        self.draw_frame_btn.setEnabled(self.frame_enabled and self.bbox is not None)
        
        if self.frame_enabled:
            self.log_progress("✓ Frame mode enabled - will restrict downloads to frame boundary")
            if self.bbox:
                self.draw_frame_on_map()
        else:
            self.log_progress("Frame mode disabled")
            self.remove_frame_from_map()
    
    def on_frame_orientation_changed(self):
        """Handle frame orientation radio button change"""
        self.frame_orientation = "landscape" if self.frame_landscape_radio.isChecked() else "portrait"
        if self.frame_enabled and self.bbox:
            self.draw_frame_on_map()
            self.log_progress(f"✓ Frame orientation changed to: {self.frame_orientation.upper()}")
    
    def create_and_build_frame(self):
        """Build frame geometry from current search bbox"""
        if not self.bbox:
            QMessageBox.warning(self, "Warning", "Please search for a place first")
            return False
        
        try:
            self.frame_geometry = FrameBuilder.create_frame_geometry(
                self.bbox, 
                self.frame_orientation
            )
            self.log_progress(f"✓ Frame created ({self.frame_orientation}): 11 × 14 inches")
            return True
        except Exception as e:
            self.log_progress(f"✗ Error creating frame: {str(e)}")
            return False
    
    def draw_frame_on_map(self):
        """Visualize frame boundary on map canvas"""
        if not self.create_and_build_frame():
            return
        
        try:
            # Remove old frame visualization if exists
            self.remove_frame_from_map()
            
            # Create frame layer
            self.frame_layer = QgsVectorLayer(
                "Polygon?crs=EPSG:4326",
                "Frame (11×14 inches)",
                "memory"
            )
            
            if not self.frame_layer.isValid():
                raise Exception("Failed to create frame layer")
            
            # Add frame feature to layer
            feature = QgsFeature()
            feature.setGeometry(self.frame_geometry)
            self.frame_layer.dataProvider().addFeatures([feature])
            self.frame_layer.updateExtents()
            
            # Style frame layer with red outline
            ...
            self.frame_layer.renderer().setSymbol(symbol)
            
            # Add layer to map
            QgsProject.instance().addMapLayer(self.frame_layer)
            self.iface.mapCanvas().refresh()
            
            self.log_progress(f"✓ Frame displayed on map ({self.frame_orientation})")
            
        except Exception as e:
            self.log_progress(f"✗ Error drawing frame: {str(e)}")
            QgsMessageLog.logMessage(
                f"Frame visualization error: {str(e)}", 
                'OSM Bulk Downloader', 
                Qgis.Warning
            )
    
    def remove_frame_from_map(self):
        """Remove frame visualization layer from map"""
        if self.frame_layer and self.frame_layer.isValid():
            try:
                QgsProject.instance().removeMapLayer(self.frame_layer.id())
                self.frame_layer = None
                self.iface.mapCanvas().refresh()
            except Exception as e:
                self.log_progress(f"⚠ Warning: Could not remove frame layer: {str(e)}")
    ...
    def search_place(self):
        ...
        if self.bbox:
            ...
            # Enable frame controls since we have a valid location
            if hasattr(self, 'draw_frame_btn'):
                self.draw_frame_btn.setEnabled(self.frame_enabled)
                if self.frame_enabled:
                    self.draw_frame_on_map()
        else:
    ...
    def start_download(self):
        ...
        # Get bbox - either from frame, search, or canvas extent
        download_bbox = None
        
        if self.frame_enabled and self.frame_geometry:
            # Use frame boundary for download
            download_bbox = FrameBuilder.get_frame_bbox(self.frame_geometry)
            self.log_progress(f"Using frame boundary for download: {download_bbox}")
        elif self.use_canvas_extent.isChecked():
