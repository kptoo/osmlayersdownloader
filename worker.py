...
class DownloadWorker(QThread):
    ...
    def __init__(self, bbox, feature_configs, parent=None):
        ...
        Args:
            bbox: (south, west, north, east) tuple in WGS84
            feature_configs: list of feature config dicts (selected features)
        """
    ...
    def abort(self):
        """Request the worker to stop after the current feature."""
        self._abort = True
    ...
    def run(self):
        """Main thread entry-point — iterates features and downloads each one."""
